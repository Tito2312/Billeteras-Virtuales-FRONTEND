...
import { getAllAudits } from '../../API/admin';
...
const AdminAudit = () => {
  ...
  const filteredAudits = audits.filter(audit => {
    const matchesSearch = audit.userId?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="admin-audit-container">
      ...
      <div className="admin-audit-filters">
        ...
        <button className="refresh-btn" onClick={loadAudits}>⟳ Actualizar</button>
      </div>
          ...
          <tbody>
            {filteredAudits.map(audit => (
              <tr key={audit.id}>
                ...
                <td className="user-cell">{audit.userId ? audit.userId.substring(0, 12) + '...' : '-'}</td>
                <td className="transaction-cell">{audit.transactionId ? audit.transactionId.substring(0, 12) + '...' : '-'}</td>
                <td>
